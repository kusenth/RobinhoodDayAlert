package senthil.jayaraman.robinhood.api;
...
import senthil.jayaraman.robinhood.api.endpoint.account.methods.*;
import senthil.jayaraman.robinhood.api.endpoint.authorize.data.Token;
import senthil.jayaraman.robinhood.api.endpoint.authorize.methods.AuthorizeWithoutMultifactor;
import senthil.jayaraman.robinhood.api.endpoint.authorize.methods.LogoutFromRobinhood;
...
import senthil.jayaraman.robinhood.api.endpoint.orders.methods.CancelOrder;
import senthil.jayaraman.robinhood.api.endpoint.orders.methods.MakeLimitOrder;
import senthil.jayaraman.robinhood.api.endpoint.orders.methods.MakeLimitStopOrder;
import senthil.jayaraman.robinhood.api.endpoint.orders.methods.MakeMarketOrder;
import senthil.jayaraman.robinhood.api.endpoint.orders.methods.MakeMarketStopOrder;
import senthil.jayaraman.robinhood.api.endpoint.orders.methods.OrderHistory;
...
import senthil.jayaraman.robinhood.api.endpoint.quote.methods.GetTickerQuote;
...
import senthil.jayaraman.robinhood.api.endpoint.account.data.AccountElement;
...
public class RobinhoodApi {
    ...
    public List<OrderHistoryResults> getOrderHistory() throws RobinhoodNotLoggedInException, RobinhoodApiException, InvalidTickerException {
    	//Create the API method for this request
		ApiMethod method = new OrderHistory();
		method.addAuthTokenParameter();
		//TODO: This is a temporary fix, as the Robinhood API seems to have some features implemented, but are not used yet
		OrderHistoryListResults data = requestManager.makeApiRequest(method);
		return data.getOrderHistoryListResults();
	...
	public SecurityOrderElement cancelorder(URL cancelurl) throws InvalidTickerException, RobinhoodNotLoggedInException, RobinhoodApiException {

		//Create the API method
		ApiMethod method = new CancelOrder(cancelurl);
		method.addAuthTokenParameter();
		return requestManager.makeApiRequest(method);
	...
	public List<PositionWatch> getAccountWatch() throws RobinhoodApiException, RobinhoodNotLoggedInException {

		//Create the API method
		ApiMethod method = new GetAccountWatch();
		method.addAuthTokenParameter();

		//Return the current account positions
		PositionListWatch response = requestManager.makeApiRequest(method);
		return response.getPositionWatch();
	...
	public List<PositionElement> getAccountPositions() throws RobinhoodApiException, RobinhoodNotLoggedInException {
		...
		//accountWatchlist.forEach(a->System.out.println(a.getStockTicker()));
